...
export default function Products() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] font-[Manrope]">
      ...
      {/* ===== HERO BANNER ===== */}
      <section className="bg-[#0D0D0D] relative overflow-hidden" data-testid="products-hero">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-2 mb-6" data-testid="active-filters">
            ...
            {category && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 text-white/70 text-xs font-medium border border-white/10">
                {category}
                <button onClick={() => setFilter("category", "")} className="hover:text-white"><X size={12} /></button>
              </span>
            )}
            {search && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 text-white/70 text-xs font-medium border border-white/10">
                &quot;{search}&quot;
                <button onClick={() => setFilter("search", "")} className="hover:text-white"><X size={12} /></button>
              </span>
            )}
            <button onClick={() => setSearchParams({})} className="text-xs text-white/40 hover:text-red-600 font-medium underline underline-offset-2 ml-2 transition-colors" data-testid="clear-filters-btn">Clear All</button>
          </div>
        )}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <button onClick={() => setMobileFiltersOpen(!mobileFiltersOpen)}
            className="lg:hidden flex items-center justify-between w-full px-4 py-3 bg-white/5 border border-white/10 rounded-sm text-sm font-semibold text-white/70"
          ...
          <aside className={`lg:w-60 shrink-0 ${mobileFiltersOpen ? "block" : "hidden lg:block"}`} data-testid="product-filters">
            <div className="sticky top-20">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-white mb-4 flex items-center gap-2">
                <SlidersHorizontal size={13} className="text-[#2DD4BF]" /> Divisions
              </h3>
                        ...
                        <div className="ml-6 mt-1 mb-2 space-y-0.5 border-l-2 border-teal-100 pl-3">
                          {subCats.map((cat) => (
                            <button key={cat} onClick={() => setFilter("category", cat)}
                              className={`block w-full text-left px-2 py-1.5 rounded-sm text-xs transition-all ${category === cat ? "bg-teal-100 text-teal-800 font-semibold" : "text-white/40 hover:text-[#2DD4BF] hover:bg-white/5"}`}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-sm text-white/40">
                <span className="font-semibold text-white">{showCategories ? categories.length : total}</span>
                {showCategories ? " categories" : ` product ${total !== 1 ? "ranges" : "range"}`}
                {search && <> matching &quot;<span className="text-[#2DD4BF]">{search}</span>&quot;</>}
              </p>
              {!showCategories && (
            ...
            {loading ? (
                ...
                {categories.map((c) => {
                  ...
                  return (
                    <button key={`${c.division}-${c.category}`}
                      ...
                      className="group bg-[#0A0A0A] border border-white/[0.06] rounded-sm overflow-hidden hover:shadow-lg hover:border-[#2DD4BF]/20 transition-all duration-300 text-left"
                      ...
                      <div className="h-36 bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center overflow-hidden relative">
                        ...
                        {c.has_brochure && (
                          <div className="absolute top-3 right-3">
                            <span className="text-[9px] font-bold text-[#2DD4BF] bg-[#2DD4BF]/10 px-2 py-0.5 rounded-full border border-teal-100">PDF Available</span>
                          </div>
                        )}
                      </div>
                      <div className="p-5">
                        <h3 className="font-bold text-white group-hover:text-[#2DD4BF] transition-colors text-base leading-snug">
                          {c.category}
                        </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5" data-testid="family-grid">
                {families.map((f) => (
                  <Link key={f.family_name}
                    ...
                    <div className="h-48 bg-white/5 flex items-center justify-center overflow-hidden p-4 relative">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#2DD4BF] bg-[#2DD4BF]/10 px-2 py-0.5 rounded-full border border-teal-100">{f.division}</span>
                      </div>
                      {f.variant_count > 1 && (
                        <div className="absolute top-3 right-3">
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-white/50 bg-[#0A0A0A] px-2 py-0.5 rounded-full border border-white/10 shadow-sm">
              ...
              <div className="space-y-3" data-testid="family-list">
                {families.map((f) => (
                  <Link key={f.family_name}
                    to={f.variant_count > 1 ? `/products/family/${encodeURIComponent(f.family_name)}` : `/products/${f.id}`}
                    className="group flex items-center gap-5 bg-[#0A0A0A] border border-white/[0.06] rounded-sm p-4 hover:shadow-lg hover:border-[#2DD4BF]/20 transition-all duration-300"
                    data-testid={`product-family-list-${f.id}`}>
                    <div className="w-20 h-20 bg-white/5 rounded-sm flex items-center justify-center shrink-0 overflow-hidden p-2">
                      {f.images && f.images.length > 0 ? (
                    ...
                    <div className="text-right shrink-0 hidden sm:block">
                      {f.variant_count > 1 && (
                        <span className="inline-flex items-center gap-1 text-xs font-medium text-white/40 bg-white/5 px-2.5 py-1 rounded-full border border-white/10">
            ...
            {!showCategories && pages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-10" data-testid="pagination">
                <button onClick={() => goPage(page - 1)} disabled={page <= 1}
                  className="flex items-center gap-1 px-4 py-2.5 border border-white/10 rounded-sm text-sm font-medium text-white/50 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                  ...
                  {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
                    ...
                    return (
                      <button key={pageNum} onClick={() => goPage(pageNum)}
                        className={`w-10 h-10 text-sm font-medium rounded-sm transition-all ${pageNum === page ? "bg-[#0D0D0D] text-white shadow-md" : "text-white/50 hover:bg-white/5"}`}
