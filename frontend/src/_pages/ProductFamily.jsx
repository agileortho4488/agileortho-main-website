...
export default function ProductFamily() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] font-[Manrope]">
      ...
      {/* ===== DARK HERO ===== */}
      <section className="bg-[#0D0D0D] relative overflow-hidden" data-testid="family-hero">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 lg:py-14">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-20 bg-white/5 border border-white/[0.06] rounded-sm overflow-hidden" data-testid="family-image">
              ...
              <div className="px-5 py-4 border-t border-white/[0.06] flex flex-wrap gap-2">
                {[family.division, family.category, family.material].filter(Boolean).map((t) => (
                  <span key={t} className="text-xs font-medium text-white/40 bg-[#0A0A0A] border border-white/10 px-3 py-1 rounded-full">{t}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-6" data-testid="family-info">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#2DD4BF] bg-[#2DD4BF]/10 border border-teal-100 px-2.5 py-1 rounded-full" data-testid="family-division">
                {family.division}
              </span>
              ...
              <span className="inline-flex items-center gap-1 text-[10px] font-bold text-white/50 bg-[#D4AF37]/10 border border-[#D4AF37]/20 px-2.5 py-1 rounded-full">
                <Layers size={10} /> {family.variant_count} variants
              </span>
            </div>

            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white tracking-tight leading-tight" data-testid="family-name">
              {decodedName}
            </h1>

            <p className="text-sm sm:text-base text-white/50 leading-relaxed" data-testid="family-description">
              {family.description}
            </p>
            ...
            <div className="space-y-3 pt-2" data-testid="family-cta">
              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={() => setShowQuoteForm(true)} className="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 bg-teal-600 text-white text-sm font-bold rounded-sm hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20" data-testid="request-quote-btn">
                  <Mail size={16} /> Request Bulk Quote
                </button>
                <a href={`https://wa.me/917416521222?text=${encodeURIComponent(`Hi, I'm interested in the ${decodedName} range. Can you share pricing and availability?`)}`} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 bg-[#25D366] text-white text-sm font-bold rounded-sm hover:bg-[#1DA851] transition-all shadow-lg shadow-[#25D366]/20" data-testid="whatsapp-enquiry-btn">
                  <MessageCircle size={16} /> WhatsApp Enquiry
                </a>
              </div>
              <div className="flex gap-3">
                <a href="tel:+917416521222" className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-white/10 text-white/50 text-sm font-semibold rounded-sm hover:bg-white/5 transition-colors" data-testid="call-btn">
        ...
        <div className="mt-14" data-testid="variants-section">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">Available Variants & Sizes</h2>
              <p className="text-sm text-white/40 mt-1">{variants.length} SKUs in this product range</p>
            </div>
            {variants.length > 5 && (
              <div className="relative w-full sm:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 border border-white/10 rounded-sm text-sm focus:outline-none focus:border-teal-500 transition-colors"
          ...
          <div className="rounded-sm overflow-hidden border border-white/10 bg-[#0A0A0A]" data-testid="variants-table-wrapper">
              ...
              <table className="w-full text-sm" data-testid="variants-table">
                ...
                <tbody>
                  {filteredVariants.map((v, i) => (
                    <tr
                      key={v.id}
                      className={`${i % 2 === 0 ? "bg-[#0A0A0A]" : "bg-white/5/50"} border-t border-white/[0.06] hover:bg-[#2DD4BF]/10/30 transition-colors`}
                      data-testid={`variant-row-${v.sku_code}`}
                    >
                      <td className="px-5 py-3 font-medium text-white/90 max-w-[300px]">
                        <span className="line-clamp-1">{v.product_name}</span>
                      </td>
                      <td className="px-5 py-3 font-mono text-xs text-white/40 whitespace-nowrap">{v.sku_code}</td>
                      {specColumns.map(col => (
                        <td key={col} className="px-5 py-3 text-white/50 whitespace-nowrap">
                      ...
                      <td className="px-5 py-3 text-center">
                        <Link
                          to={`/products/${v.id}`}
                          className="inline-flex items-center gap-1 text-xs text-[#2DD4BF] font-semibold hover:text-[#2DD4BF] transition-colors"
          ...
          <div className="bg-[#0A0A0A] rounded-sm shadow-2xl w-full max-w-md overflow-hidden" data-testid="quote-modal">
            ...
            <form onSubmit={handleSubmitQuote} className="p-6 space-y-3" data-testid="quote-form">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={submitting} className="flex-1 px-4 py-2.5 bg-teal-600 text-white text-sm font-bold rounded-sm hover:bg-teal-700 disabled:opacity-50 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setShowBrochureForm(false); }}>
          <div className="bg-[#0A0A0A] rounded-sm shadow-2xl w-full max-w-md overflow-hidden" data-testid="brochure-modal">
            ...
            >
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowBrochureForm(false)} className="flex-1 px-4 py-2.5 border border-white/10 text-white/50 text-sm font-semibold rounded-sm hover:bg-white/5 transition-colors">Cancel</button>
                <button type="submit" disabled={brochureSubmitting} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-teal-600 text-white text-sm font-bold rounded-sm hover:bg-teal-700 disabled:opacity-50 transition-colors">
